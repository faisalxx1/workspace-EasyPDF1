...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!options || !options.rotation || ![90, 180, 270].includes(options.rotation)) {
      return NextResponse.json(
        { error: 'Valid rotation (90, 180, or 270 degrees) is required' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      const result = await pdfProcessor.rotatePDF(file.filePath, options)
